...
import { scenePlaceLabel, scenePrecisionLabel, sceneReadingCue } from "../map/nodeScenePresentation";
...
export default function NodeSceneCartouche({ node, scene, placeLabel }: Props) {
  return (
    <aside className="node-scene-cartouche" role="note" aria-label={`节点 ${node.seq} 地图读图提示`}>
      ...
      <p className="node-scene-cartouche-place">
        <span className="node-scene-cartouche-place-label">地点</span>
        <span className="node-scene-cartouche-place-value">{placeLabel || scenePlaceLabel(scene, node)}</span>
      </p>
