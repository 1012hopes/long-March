...
本文件用于开工前验证，不代表正式产品技术选型已经放行。下列候选路线保留；当前可演示原型已使用 React + TypeScript + Vite + MapLibre GL 验证节点地图、离线地形和渐进 3D。

## 1. 候选技术路线

- React + TypeScript + Vite；
- OpenLayers 仍是天地图、CGCS2000 与自定义投影 Spike 的候选；当前原型使用 MapLibre GL；
...
- 不引入 Cesium、后端数据库或大模型；原型仅使用 MapLibre 的按需 DEM 地形作为可关闭增强，不作为主流程依赖。

正式选型判断不变：CGCS2000、WMTS、天地图与合规能力比炫 3D 更重要。MapLibre 原型结果不能替代 S1 合规接入验证。

### 当前原型技术证据（2026-09-04）

- 本地全局与九节点地形图可先于在线 DEM 显示；
- 全局 100/50 米等高线已移出首屏；
- 在线 3D 支持后台预取、显式开启、取消、失败与重试；
- 九节点场景、六类关键水系和标注来源有自动校验；
- 390×844、1280×720、1440×900、1920×1080 浏览器截图均无横向溢出；
- 当前生产 JS 约 406KB gzip，仍高于 300KB 预算，性能门禁未通过；
- 未完成低性能安卓真机 30fps、断网刷新/PWA、天地图对齐与实体投影观看测试。
